...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        # input.shape = (50,3,64,64)
        
        if not stochastic:
            return inputs * self.incl_prob # keeping constant the magnitude of the layer

        if self.share_across_batch: # one dropout mask
            # Generate a single dropout mask and apply it to the last 3 arrays
            self.mask = (self.rng.uniform(0, 1, size=inputs.shape[1:]) < self.incl_prob).astype(float)
        else: # generate the first dimentional
            # Generate a single dropout mask for each input
            self.mask = (self.rng.uniform(0, 1, size=inputs.shape) < self.incl_prob).astype(float)

        # Apply the dropout mask to the inputs
